...
class Parser:
	...
	def com_expr(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected int, float, identifier, '+', '-', '(', 'INVER'"
			))
	...
	def expr(self):
		...
		node = res.register(self.bin_op(self.com_expr, ((TT_KEYWORD, "WITH"), (TT_KEYWORD, "OR"))))

		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected 'VAL', 'CONST', int, float, identifier, '+', '-' or '('"
			))
